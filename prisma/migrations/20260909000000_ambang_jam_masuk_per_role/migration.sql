-- Ambang jam masuk berbeda per jabatan: Kepala Cabang 07.30, Karyawan 07.55.
--
-- Sebelumnya satu ambang (07.40) berlaku untuk semua orang lewat
-- `hv_work_start_minutes()` (0 argumen). Sekarang ambangnya tergantung
-- `custom_role.name` si pemilik baris presensi, jadi fungsinya butuh
-- argumen itu — pasangan TypeScript-nya ada di
-- `classifyWorkStartRole`/`workStartMinutesFor` di src/lib/attendance-time.ts.
--
-- "Kepala & Kasir" ikut dianggap Kepala Cabang, sama seperti pengelompokan
-- di ROLE_PERMISSION_MAP (src/lib/permissions.ts) yang memetakan dua nama itu
-- ke izin yang sama.
--
-- Fungsi BARU dibuat dulu (signature 1 & 3 argumen, bukan menimpa yang 0 & 2
-- argumen) supaya view bisa dipindah ke panggilan barunya SEBELUM fungsi lama
-- dihapus — urutan sebaliknya akan gagal karena view masih bergantung padanya.
CREATE OR REPLACE FUNCTION hv_work_start_minutes(role_name text)
RETURNS integer
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT CASE WHEN upper(trim(role_name)) IN ('KEPALA CABANG', 'KEPALA & KASIR')
    THEN 7 * 60 + 30
    ELSE 7 * 60 + 55
  END
$$;

-- Apakah satu baris presensi terlambat, dengan ambang bergantung jabatan.
--
...
CREATE OR REPLACE FUNCTION hv_is_late(status "AttendanceStatus", check_in timestamp, role_name text)
RETURNS boolean
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT CASE
    WHEN check_in IS NOT NULL THEN hv_checkin_minutes(check_in) > hv_work_start_minutes(role_name)
    ELSE status = 'LATE'::"AttendanceStatus"
  END
$$;

-- ── hv_attendance_monthly ──────────────────────────────────────────────────
-- Identik dengan versi sebelumnya, kecuali hv_is_late() sekarang menerima
-- cr.name sebagai argumen ketiga — join ke custom_role sudah ada di view ini
-- sejak awal (dipakai kolom role_name), jadi tidak perlu join baru.
CREATE OR REPLACE VIEW hv_attendance_monthly AS
 SELECT a."userId" AS user_id,
    ...
    count(*) FILTER (WHERE a.status = ANY (ARRAY['PRESENT'::"AttendanceStatus", 'LATE'::"AttendanceStatus"]) AND NOT hv_is_late(a.status, a."checkIn", cr.name)) AS present_days,
    count(*) FILTER (WHERE a.status = ANY (ARRAY['PRESENT'::"AttendanceStatus", 'LATE'::"AttendanceStatus"]) AND hv_is_late(a.status, a."checkIn", cr.name)) AS late_days,
...
-- ── hv_payroll_monthly ─────────────────────────────────────────────────────
-- Sama seperti di atas: hanya panggilan hv_is_late() yang berubah, di dalam
-- CTE `att`. `att` tidak punya cr.name sendiri, jadi join ke custom_role
-- ditambahkan DI DALAM CTE itu — bukan memindah agregasinya ke `base`, yang
-- akan mengubah bentuk GROUP BY yang sudah ada.
 ...
 WITH att AS (
         SELECT a."userId",
           ...
           FROM "Attendance" a
             ...
             LEFT JOIN custom_role cr ON cr.id = au."customRoleId"
...
-- Fungsi lama (0 dan 2 argumen) sudah tidak dipanggil view mana pun sejak
-- CREATE OR REPLACE di atas — aman dihapus.
DROP FUNCTION IF EXISTS hv_work_start_minutes();
DROP FUNCTION IF EXISTS hv_is_late("AttendanceStatus", timestamp);

-- Koneksi read-only rule engine (DATABASE_VIEW_ONLY_URL) memanggil fungsi baru
-- ini langsung dari SQL rule `denda_keterlambatan` (lihat
-- prisma/scripts/apply-jam-masuk.ts) DAN lewat view di atas.
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'oc_pvi_reader') THEN
    GRANT EXECUTE ON FUNCTION hv_work_start_minutes(text) TO oc_pvi_reader;
    GRANT EXECUTE ON FUNCTION hv_is_late("AttendanceStatus", timestamp, text) TO oc_pvi_reader;
  END IF;
END $$;
